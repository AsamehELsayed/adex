...
const Footer = () => {
  ...
  const { content: displayContent } = useLocalizedContent("footer-section", defaultFooterContent);
  const content = displayContent || defaultFooterContent;

  const getSocialIcon = (iconName) => {
    return socialIconMap[iconName?.toLowerCase()] || socialIconMap.linkedin;
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          ...
          <div>
            <h4 className="font-serif text-lg mb-6">
              {content.navigateLabel || "Navigate"}
            </h4>
